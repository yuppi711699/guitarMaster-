...
import backgroundVideoOfGame from './videos/videoplayback.mp4'

function App() {
  return (
    <div className="App">
      <div className="gamescreen">
        <h1>Led Zeppelin - Stairway To Heaven (LIVE version) </h1>
        <video id="myBackgroundVideo">
          <source src={backgroundVideoOfGame} type="video/mp4" />
        </video>
        <button id="stateOfGame">Start</button>
        <button id="reloadOfGame">Reload</button>
...
function pageLoaded(){
  backgroundVideo = document.getElementById('myBackgroundVideo')
  startStopButton = document.getElementById('stateOfGame');
  canvas = document.getElementById("canvas");
  ctx = canvas.getContext("2d");
  ctx.font = "30px Arial";
  setTimeout(makeBlock, 0);
  window.addEventListener("keydown", keyDown);
  window.addEventListener('click', changeStateOfGame)
}
// var 
